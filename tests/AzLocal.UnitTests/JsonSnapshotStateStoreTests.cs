...
public class JsonSnapshotStateStoreTests : IDisposable
{
    private record TestValue(string Name, int Count);

    private readonly string _dir;

    public JsonSnapshotStateStoreTests()
    {
    ...
    private JsonSnapshotStateStore NewStore() =>
        new(new InMemoryConfig("AzLocal:SnapshotPath", _dir));
    ...
    public async Task SetAsync_ThenGetAsync_RoundTripsValue()
    {
        ...
        Assert.Equal(value, await store.GetAsync<TestValue>("blob/items/acct/container/widget"));
    }
    ...
    [Fact]
    public async Task SetAsync_WritesSnapshotFileToDisk()
    {
        var store = NewStore();

        await store.SetAsync("key", new TestValue("on-disk", 1));

        Assert.True(File.Exists(Path.Combine(_dir, "state.json")));
    }

    [Fact]
    public async Task Data_PersistsAcrossStoreInstances_ByReloadingSnapshotFile()
    {
        var first = NewStore();
    ...
    [Fact]
    public async Task NewStore_WithNoExistingSnapshotFile_StartsEmpty()
    {
        var store = NewStore();
